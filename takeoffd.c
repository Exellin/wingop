...
double takeoffd(double a1, double a2, double c1, double c2, double b1, double b2, double d, double z, double m, double step)
{
	...
	{
		vn = vn1;
		ai1 = a1 - (s1223cl(a1, vn, c1)/(M_PI*ar1))*(180/M_PI);
		ai2 = a2 - (s1223cl(a2, vn, c2)/(M_PI*ar2))*(180/M_PI) - (s1223cl(ai1, step, c1)/(2*M_PI*ar1) * ( ((1+(2*d/b1))/((pow(1+(2*d/b1),2)+pow((2*z/b1),2)))) + ((1-(2*d/b1))/((pow(1-(2*d/b1),2)+pow((2*z/b1),2)))) ) )*180/M_PI;
